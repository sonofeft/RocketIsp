...
F = Figure( figsize=(5,4), dpi=300, nrows=1, ncols=1,
            ...
            png_path_name= os.path.join(up_one, '_static', 'effdiv_monte_Rthrt.png') )
...
lsL = ['-','-',':']
for j,pcb in enumerate(pcbL):
    for i,Rthrt in enumerate([0.1,  10.0]):
        effL = []
        for eps in epsArr:
            eff = (100-calc_pcentLossDiv( Pc=500.0, eps=eps, Rthrt=Rthrt, pcentBell=pcb, gammaInit=1.2,
                           TcCham=5500.0, MolWt=20.0, RWTD=1.0 )) / 100.0
            effL.append( eff )
            
        curve = Curve(plot_type='semilogx', xL=epsArr, yL=effL, marker='',
                      place_labels_on_line=True, xpos_label=3.5+i, dy_placement=dypLL[j][i],
                      label='Rt=%g'%Rthrt, linecolor=CWheel(j), 
                      linestyle=lsL[i], linewidth=i+1)
        curveL.append( curve )
